...
public class MultipleExerciseMaximumNumberOfOptions implements Validator<MultipleExercise> {
	...
	private static final String DEFINITION = "if maximumNumberOfOptions exists then maximumNotionalAmount >= 0";
	...
	private ComparisonResult ruleIsApplicable(MultipleExercise multipleExercise) {
		try {
			return exists(MapperS.of(multipleExercise).<Integer>map("getMaximumNumberOfOptions", _multipleExercise -> _multipleExercise.getMaximumNumberOfOptions()));
		}
	...
	private ComparisonResult evaluateThenExpression(MultipleExercise multipleExercise) {
		try {
			return greaterThanEquals(MapperS.of(multipleExercise).<BigDecimal>map("getMaximumNotionalAmount", _multipleExercise -> _multipleExercise.getMaximumNotionalAmount()), MapperS.of(Integer.valueOf(0)), CardinalityOperator.All);
		}
		catch (ModelObjectValidator.ModelObjectValidationException ex) {
			return ComparisonResult.failure(ex.getErrors());
		}
